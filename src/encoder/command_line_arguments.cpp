...
{
    if (m_components_count != 1 && m_components_count != 3 && m_components_count != 4) {
        throw std::invalid_argument("Unsupported components count");
    }

    if (m_quality < 1 || m_quality > 100) {
        throw std::invalid_argument("The quality value should be from 1 to 100 inclusive");
    }
}
...
std::size_t CommandLineArguments::get_width() const
{
    return m_width;
}

std::size_t CommandLineArguments::get_height() const
{
    return m_height;
}
...
std::size_t CommandLineArguments::get_quality() const
{
    return m_quality;
}
...
bool CommandLineArguments::validate(const int argc, const char * argv[])
{
    if (argc < 7) {
        ...
        throw std::invalid_argument("There are too few command line arguments\nUsage: " + app_name + " <input_file> <output_file> <width> <height> <components> <quality>");
    }
    return true;
}
